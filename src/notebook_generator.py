...
def create_rich_notebook(filename: str, title: str, module_id: str, sections: List[Dict[str, Any]], prev_next: Tuple[str, str]) -> None:
    ...
    if prev_link: nav_html += f"[← Previous: {prev_link}]({prev_link})"
    nav_html += "</span><span>[Table of Contents](00_introduction.ipynb)</span><span>"
    if next_link: nav_html += f"[Next: {next_link} →]({next_link})"
    nav_html += "</span></div>"
...
def get_module_sections(module_id: str) -> List[Dict[str, Any]]:
    if module_id == "M0_1":
        return [
            {'type': 'markdown', 'content': "# #01 Pythonの基礎（地球科学の例題で学ぶ）\n\nプログラミングの第一歩として、変数の使い方や計算の方法を学びます。"},
            {'type': 'markdown', 'content': "### 1. 変数と計算\n岩石の密度を計算してみましょう。密度($\\rho$)は質量($m$)を体積($V$)で割ったものです。\n\n$$\\rho = m / V$$"},
            {'type': 'code', 'content': "# 質量 (g)\nmass = 120.5\n# 体積 (cm3)\nvolume = 45.2\n# 密度の計算\ndensity = mass / volume\nprint(f\"岩石の密度は {density:.2f} g/cm3 です\")"},
            {'type': 'exercise', 'content': '質量 250.0g、体積 80.0cm3 のサンプルの密度を計算し、表示してください。', 'expected_output': '岩石の密度は 3.12 g/cm3 です'}
        ]
    elif module_id == "M0_2":
        return [
            {'type': 'markdown', 'content': "# #02 Pandasの基礎（表データの扱い方）\n\n地球科学で最も多用されるライブラリ「Pandas」を使って、CSVファイルを読み込み、データを眺める方法を学びます。"},
            {'type': 'code', 'content': "import pandas as pd\n# データの読み込み\ndf = pd.read_csv('../data/raw_tabular/petrology.csv')\n# 最初の5行を表示\ndisplay(df.head())\n# データの要約統計量（平均や最大値など）を表示\ndisplay(df.describe())"},
            {'type': 'exercise', 'content': '読み込んだデータフレーム `df` の列名（columns）を一覧表示してください。', 'expected_output': "Index(['Sample_ID', 'SiO2', 'MgO', 'FeO', 'Cr_ppm'], dtype='object')"}
        ]
    elif module_id == "M1":
        return [
            {'type': 'markdown', 'content': "# #03 岩石化学データ解析\n\n## 0. 背景：マグマの進化とMg#\nマグマの冷却に伴う結晶分化作用を理解するための指標、Mg#（マグネシウム数）を算出します。"},
        ...
        return [
            {'type': 'markdown', 'content': "# #09 地球ダイナミクス・シミュレーション\n\n## 0. 背景：常微分方程式による物質循環\n地球内部の物質移動や火山噴火の物理プロセスを、数値シミュレーションで再現します。"},
...
def generate_all_notebooks() -> None:
    modules: List[Tuple[str, str, str]] = [
        ("01_python_basics.ipynb", "Pythonの基礎", "M0_1"),
        ("02_pandas_basics.ipynb", "Pandasの基礎", "M0_2"),
        ("03_tabular_petrology.ipynb", "岩石化学", "M1"),
        ("04_timeseries_granulometry.ipynb", "時系列解析", "M2"),
        ("05_geospatial_mapping.ipynb", "地形解析", "M3"),
        ("06_image_geometry.ipynb", "画像解析", "M4"),
        ("07_volume_gui.ipynb", "3D解析", "M5"),
        ("08_geochemistry_clustering.ipynb", "地球化学クラスタリング", "M6"),
        ("09_earth_dynamics_simulation.ipynb", "地球シミュレーション", "M7"),
        ("10_field_data_automation.ipynb", "野外調査自動化", "M8"),
        ("11_mineral_spectroscopy.ipynb", "鉱物分光解析", "M9"),
        ("12_bigdata_timespace.ipynb", "ビッグデータ解析", "M10")
    ]
    ...
    for i, (filename, title, m_id) in enumerate(modules):
        ...
        create_rich_notebook(filename, f"Module {m_id}: {title}", m_id, sections, (prev_f, next_f))
